...
export default {
  ...
  argTypes: {
    ...
    items: {
      description:
        ' label:Adicione uma label;  isDivider?:adicione uma linha de divisão entre os itens; icon?:adicione um ícone(Icones do FontAwsome, passar somente o nome dele) obs:Essa opção é apenas dísponível para variant (icon) ; value?:adicione um valor(se for do tipo checkbox,ou radio) e somente é possível utilizar nas variants checkbox,radio,filter; subItens?:se a variant for(checkbox ou se o tipo do item menu for checkbox,é possível adicionar a este um subitens) isTitleFilterBy?:se for a variant (filter) adicione um titulo "Filter by" acima do item que quiser typeInput?:se a variant for (filter) adicione o tipo do elemento(radio,checkbox)(opcional)  checked?:sete algum valor selecionado diretamente nos campos do tipo radio,checkbox',
    },
    onChange: {
      type: "function",
      description:
        "Verifique por meio dessas propriedades e acompanhe todas as alterações como (valor),(item alterado) e o index do item alterado,bem como verifique se aquele elemento é um subitem ou não",
    },
  },
